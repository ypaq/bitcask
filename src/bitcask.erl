...
-define(debugMsg2(Msg), error_logger:info_msg(Msg, [])).
-define(debugFmt2(Msg, Args), error_logger:info_msg(Msg, Args)).
...
slow_worker() ->
    ?debugMsg2("slow_worker started, waiting for owner/values\n"),
    {Owner, Values} =
        receive
            {owner, O, Vs} -> {O, Vs}
        end,
    ?debugMsg2("slow worker got owner/values\n"),
    ...
    ?debugMsg2("slow worker will open\n"),
    B = bitcask:open("/tmp/bc.test.unfrozenfold"),
    Owner ! ready,
    ?debugMsg2("slow worker will fold\n"),
    ...
    ?debugFmt2("slow worker finished~n", []),
    bitcask:close(B),
    ?debugMsg2("slow worker closed bitcask\n").
...
fold_visits_unfrozen_test(RollOver) ->
    ...
    try
        ...
        ?debugMsg2("Final fold\n"),
        %% Check we see the updated fold
        L2 = fold(B, CollectAll, [], -1, -1, false),
        ?assertEqual([{<<"k2">>,<<"v2-2">>},
                      {<<"k3">>,<<"v3">>},
                      {<<"k4">>,<<"v4">>}], lists:sort(L2)),
        ?debugFmt2("got past the asserts??~n", [])
    after
        bitcask:close(B),
...
merge_batch_test() ->
    ...
    try
        ...
        NM1 = bitcask:needs_merge(B),
        ?debugMsg2("NM1\n"),
        ?assertEqual({true, {ExpData([7, 9, 10]), []}}, NM1),
        ?debugMsg2("Will merge 1\n"),
        ok = bitcask:merge(Dir, [], [BFile(7), BFile(9)]),
        ?debugMsg2("Merged1\n"),
        NM2 = bitcask:needs_merge(B),
        ?debugMsg2("NM2\n"),
        ?assertEqual({true, {ExpData([10]), []}}, NM2),
        ?debugMsg2("Will merge 2\n"),
        ok = bitcask:merge(Dir, [], [BFile(10)]),
        ?debugMsg2("Merged2\n"),
        NM3 = bitcask:needs_merge(B),
        ?debugMsg2("NM3\n"),
        ?assertEqual({true, {ExpData([12, 13]), []}}, NM3),
        ?debugMsg2("Will merge 3\n"),
        ok = bitcask:merge(Dir, [], [BFile(12)]),
        ?debugMsg2("Merged3\n"),
        NM4 = bitcask:needs_merge(B),
        ?debugMsg2("NM4\n"),
        ?assertEqual({true, {ExpData([13]), []}}, NM4),
        ?debugMsg2("Will merge 4\n"),
        ok = bitcask:merge(Dir, [], [BFile(13)]),
        ?debugMsg2("Merged4\n"),
        NM5 = bitcask:needs_merge(B),
        ?debugMsg2("NM5\n"),
        ?assertEqual({true, {ExpData([14]), []}}, NM5),
